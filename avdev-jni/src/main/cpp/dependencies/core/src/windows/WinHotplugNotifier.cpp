...
namespace avdev
{
	...
	DWORD WINAPI WinHotplugNotifier::run(void * context)
	{
		...
		if (!notifier->initializeWindow()) {
			LOGDEV_ERROR("Initialize notification window failed.");
			return 0;
		}
		if (!notifier->registerForDeviceNotification()) {
			LOGDEV_ERROR("Register for device notification failed: hWnd = %p.", notifier->hWnd);
			return 0;
		}
	...
	bool WinHotplugNotifier::registerForDeviceNotification()
	{
		...
		for (GUID & devInterface : deviceInterfaces) {
			...
			if (handle != nullptr) {
				...
				LOGDEV_ERROR("Register device notification failed.");
			}
		}
	...
	void WinHotplugNotifier::onDeviceChange(HWND hWnd, WPARAM wParam, LPARAM lParam)
	{
		...
		wchar_t buffer[512];
		wcsncpy_s(buffer, devInterface->dbcc_name, 512);

		std::wstring symLink(buffer);
		std::transform(symLink.begin(), symLink.end(), symLink.begin(), ::tolower);

		switch (wParam) {
			case DBT_DEVICEARRIVAL:
				win->onDeviceConnected(symLink);
				break;

			case DBT_DEVICEREMOVECOMPLETE:
				win->onDeviceDisconnected(symLink);
				break;
		}
	}
